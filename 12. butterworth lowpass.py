# 12. butterworth lowpass.py
# Fourier transform - Butterworth lowpass filter
...
# Apply filter here:
# Butterworth Low Pass Filter (BLPF) centered at image center (h//2, w//2)
D0 = 30
n = 2
h, w = magnitude_spectrum_ac.shape
center_x, center_y = h // 2, w // 2

H = np.zeros_like(magnitude_spectrum_ac)

# apply Butterworth Lowpass
for i in range(h):
    for j in range(w):
        D = ((i - center_x)**2 + (j - center_y)**2)**0.5
        H[i, j] = 1 / (1 + (D / D0)**(2 * n))
